...
- 一致 → 直接使用缓存，输出 `[风格检测] (缓存命中)` + 缓存内容，跳到步骤二。
...
每完成一个有意义的动作（修了一个 bug、改了一段代码、调了一个配置），**立即**过一遍。不是交付前才检查，是每个动作后都检查。
...
1. **画依赖链** — 涉及多组件时，画出完整链路 `[全链路] 请求 → A → B → C → D`，标注每跳状态
2. **自底向上验证** — 从最底层开始，逐层确认。上层的错误经常是底层问题的级联症状
3. **冰山检查** — 用户指的地方是冰山一角。上下文 50 行内还有什么相关代码？
4. **证据链完整** — 每个"✓"都对应一条实际验证输出，不是"我觉得"
...
**错误记忆**（满足任一即记录）：
1. 同一错误在**单次会话中出现 ≥2 次** → 记录到 `.claude/p8-lessons.md`
2. 用户明确纠正了做法（"不要这样做"、"应该那样做"）→ 立即记录
3. 质量自检发现了本应避免的问题 → 记录

**解决方案记忆**（满足任一即记录）：
1. 应急流程成功解决卡壳 → 记录解决方案
2. 经过 ≥3 次尝试才找到正确方案 → 记录解决路径
3. 发现了一个高效的排查方法 → 记录

### 写入规则

- 写入前先读取现有文件，检查是否已有同类记录
- 已有同类错误 → 递增 `发生次数`，更新 `最近发生` 日期
- 已有同类解决方案 → 更新内容，不重复添加
- 每条记录保持简洁，不超过 5 行关键信息
- 文件超过 50 条记录时，按 `发生次数` 排序，淘汰发生次数 = 1 且超过 30 天的记录
